document.addEventListener('DOMContentLoaded', () => {
    ...
    if(window.location.href != 'http://localhost/UTU-project/interfaz/private-html/backoffice/backoffice-admproduct.php'){
        fetch(url)
            .then(response => response.json())
            .then(data => {
                const productsContainer = document.querySelector(".modificarProducto");
        ...
        fetch('/UTU-project/logica/productos/showProductsBackOffice.php')
            .then(response => response.json())
            .then(data => {
                const productsContainer = document.querySelector(".modificarProducto ");
                productsContainer.innerHTML = '';
                data.forEach(product => {
                    ...
                    const template = `
                <div class="card product col m-1" style="width: 18rem;" id="${product.idProducto}">
                    <img src="/UTU-project/persistencia/assets/${product.imagenProducto}" class="card-img-top" alt="..." height="200px">
                    <div class="card-body">
                        <h2>${product.nombreProducto}</h2>
                        <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                        <button class="btn btn-outline-success me-2 modificar" id="${product.idProducto}" data-bs-toggle="modal" data-bs-target="#staticBackdrop">Modify product</button>
